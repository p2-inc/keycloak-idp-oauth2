    ...
    getConfigProperties().stream()
        ...
        .forEach(
            config -> {
              ...
              config.setHelpText(
                  "Path of field in OAuthProvider provider User Profile JSON data to get value from. You can use dot notation for nesting . A array index can be accessed with the dot notation as well. Eg. 'contact.address.0.country'.");
            });
  }
